        ...
        for (this.lineNum = 0; this.lineNum < this.lines.length; this.lineNum++) {
            if (this.lines[this.lineNum].staff) {
                for (this.staffNum = 0; this.staffNum < this.lines[this.lineNum].staff.length; this.staffNum++) {
                    ...
                    for (this.voiceNum = 0; this.voiceNum < this.lines[this.lineNum].staff[this.staffNum].voices.length;
                    this.voiceNum++) {
...
window.ABCJS.parse.Parse = function() {
    ...
    var s = [["<", "accent"], [">", "accent"], ["tr", "trill"], ["<(", "crescendo("], ["<)", "crescendo)"], [">(", "diminuendo("], [">)", "diminuendo)"], ["plus", "+"], ["emphasis", "accent"]];
    var h = function(A, C) {
...
ABCJS.write.StaffGroupElement.prototype.layout = function(g, p, a) {
    this.spacingunits = 0;
    this.minspace = 1000;
    var l = p.paddingleft * p.scale;
    var b = 0;
    for (var e = 0; e < this.voices.length; e++) {
        if (this.voices[e].header) {
            var q = p.paper.text(100 * p.scale, -10 * p.scale, this.voices[e].header).attr({
                "font-size": 12 * p.scale,
                "font-family": "serif",
                "font-weight": "bold"
            });
    ...
    while (!this.finished()) {
        ...
        for (e = 0; e < n.length; e++) {
            if (n[e].getNextX() > l) {
                l = n[e].getNextX();
                k = n[e].getSpacingUnits();
                o = n[e].spacingduration
            }
        }
        this.spacingunits += k;
        this.minspace = Math.min(this.minspace, k);
        for (e = 0; e < n.length; e++) {
            var f = n[e].layoutOneItem(l, g);
            var r = f - l;
            if (r > 0) {
                l = f;
                for (var c = 0; c < e; c++) {
    ...
    for (e = 0; e < this.voices.length; e++) {
        if (this.voices[e].getNextX() > l) {
            l = this.voices[e].getNextX();
            k = this.voices[e].getSpacingUnits()
        }
    }
    this.spacingunits += k;
    this.w = l;
...
ABCJS.write.VoiceElement.prototype.draw = function(f, g) {
    var d = this.w - 1;
    f.y = this.staff.y;
    f.staffbottom = this.staff.bottom;
    this.barbottom = f.calcY(2);
    if (this.header) {
        var e = 12 - (this.voicenumber + 1) * (12 / (this.voicetotal + 1));
        var a = (this.startx - f.paddingleft) / 2 + f.paddingleft;
        a = a * f.scale;
        f.paper.text(a, f.calcY(e) * f.scale, this.header).attr({
    ...
    for (var b = 0, c = this.children.length; b < c; b++) {
        this.children[b].draw(f, (this.barto || b === c - 1) ? g : 0)
    }
    window.ABCJS.parse.each(this.beams, function(h) {
        h.draw(f)
    });
    window.ABCJS.parse.each(this.otherchildren, function(h) {
        h.draw(f, this.startx + 10, d)
    })
};
...
ABCJS.write.EndingElem.prototype.draw = function(c, d, b) {
    ...
    if (this.anchor1) {
        ...
        c.printText(d + 5 * c.scale, 18.5, this.text).attr({
...
ABCJS.write.BeamElem.prototype.drawStems = function(r) {
    ...
    for (var g = 0, q = this.elems.length; g < q; g++) {
        ...
        for (var f = a.length - 1; f >= 0; f--) {
            if (g === q - 1 || ABCJS.write.getDurlog(this.elems[g + 1].abcelem.duration) > (-f - 4)) {
                var k = p;
                var h = l + n * (f + 1);
                if (a[f].single) {
                    k = (g === 0) ? p + 5 : p - 5;
...
ABCJS.write.Layout.prototype.printNote = function(L, d, S) {
    ...
    var g = new ABCJS.write.AbsoluteElement(L, r * this.tripletmultiplier, 1);
    if (L.rest) {
        var k = 7;
        ...
        for (I = (V === "down") ? L.pitches.length - 2 : 1; (V === "down") ? I >= 0 : I < L.pitches.length; I = (V === "down") ? I - 1 : I + 1) {
        ...
        for (O = 0; O < L.chord.length; O++) {
            ...
            default:
                if (L.chord[O].rel_position) {
                    ...
                    g.addChild(new ABCJS.write.RelativeElement(L.chord[O].name, H, 0, G, {
                        type: "text"
                    }))
                }
            }
...
ABCJS.write.Layout.prototype.printDecoration = function(l, k, t, n, a, r, f) {
    ...
    if (y) {
        var A = new ABCJS.write.CrescendoElem(y.start, y.stop, "<");
        this.voice.addOther(A)
    }
    if (s.length > 0) {
...
ABCJS.write.Printer.prototype.printTempo = function(t, k, q, h, e, j) {
    ...
    if (t.duration) {
        ...
        var w = 14.5;
        var a = t.duration[0];
        var n = new ABCJS.write.AbsoluteElement(t, a, 1);
